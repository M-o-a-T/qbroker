    ...
    async with unit(1) as unit1:
        async with unit(2) as unit2:
            ...
            with pytest.raises(SyntaxError):
                await unit2.rpc(
                    "qbroker.debug.app.test.debug.debug.1", dict(cmd="eval", code="blubb(")
                )
            res = await unit2.rpc("qbroker.debug.app.test.debug.debug.1", dict(cmd="ping"))
            assert res == "pong", res
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1", dict(cmd="eval", code="foo")
            )
            assert res == "bar", res
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1", dict(cmd="eval", code="baz", baz="quux")
            )
            assert res == "quux", res
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1",
                dict(cmd="eval", code="what='ever'", mode="single", what="duh")
            )
            assert res in (None, "")
            with pytest.raises(NameError):
                res = await unit2.rpc(
                    "qbroker.debug.app.test.debug.debug.1", dict(cmd="eval", code="what")
                )
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1",
                dict(cmd="eval", code="what='ever'", mode="single")
            )
            ...
            await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1",
                dict(cmd="eval", mode="exec", code="""\
def hello():
    return "yo"
""")
            )
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.debug.1", dict(cmd="eval", code="hello()")
            )
            assert res == "yo", res
...
async def test_reconnect():
    ...
    async with unit(1) as unit1:
        async with unit(2) as unit2:
            ...
            await unit2.rpc(
                "qbroker.debug.app.test.debug.reconnect.1",
                cmd="eval",
                mode="exec",
                code="""\
def test_conn(c):
    ...
    """
            ...
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.reconnect.1", cmd="eval", code="test_conn(conn)"
            )
            assert res == 1
            res = await unit2.rpc(
                "qbroker.debug.app.test.debug.reconnect.1",
