...
class Connection(object):
    ...
    async def _setup_one(
            self, name, exchange_mode, callback=None, q=None, route_key=None, exclusive=None,
            alt=None
    ):
        """\
        ...
        try:
            await ch.channel.exchange_declare(
                cfg.exchanges[name],
                exchange_mode,
                auto_delete=False,
                durable=True,
        ...
        if q is not None:
            ...
            ch.queue = await ch.channel.queue_declare(
                cfg.queues[name] + q,
                auto_delete=True,
                passive=False,
                exclusive=exclusive,
            ...
            if route_key is not None:
                logger.debug(
                    "Chan %s: bind %s %s %s", ch.channel, cfg.exchanges[name], route_key,
                    ch.queue['queue']
                )
                await ch.channel.queue_bind(
                    ch.queue['queue'], cfg.exchanges[name], routing_key=route_key
                )
        else:
    ...
    async def setup_channels(self):
        ...
        await self._setup_one(
            "rpc", 'topic', self._on_rpc_in, b.uuid, 'qbroker.uuid.' + b.uuid, alt="dead"
        )
        await self._setup_one("reply", 'direct', self._on_reply, b.uuid, b.uuid)
        if b.cfg.handlers['dead']:
            await self._setup_one(
                "dead", 'topic', self._on_dead_rpc, "rpc", exclusive=False, route_key='#'
            )
    ...
    async def _on_reply(self, channel, body, envelope, properties):
        logger.debug(
            "read reply %s for %s: %s", envelope.delivery_tag, properties.correlation_id, body
        )
        try:
            codec = get_codec(properties.content_type)
            msg = codec.decode(body)
    ...
    async def _send(self, msg):
        ...
        await getattr(self, '_ch_' + msg.type).channel.publish(
            data, msg._c_exchange, msg.dest, properties=props
        )
